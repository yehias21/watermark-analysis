...
RUN_SUBDIRS = ('models', 'plots', 'logs', 'samples')
WANDB_PROJECT = 'new_adaptive-attack'
...
def create_run_directory() -> str:
    ...
    for subdir in tqdm(RUN_SUBDIRS, desc="Creating directories"):
...
def save_sample_images(x_wm: torch.Tensor, recon_x: torch.Tensor, epoch: int, run_dir: str) -> None:
    ...
    with tqdm(total=3, desc=f"Saving samples for epoch {epoch}") as pbar:
        ...
        n_wm = min(MAX_SAMPLES_TO_PLOT, x_wm.size(0))
        n_recon = min(MAX_SAMPLES_TO_PLOT, recon_x.size(0))
...
def train_new_adaptive_attack(batch_size=4, num_epochs=10, learning_rate=1e-5, alpha=1, beta=1,
    ...
    # Initialize W&B
    wandb.init(project=WANDB_PROJECT, name=f'config_alpha_{alpha}_beta_{beta}_mode_{mode}_dataset_{cache_dir}')
    wandb.config.update({
