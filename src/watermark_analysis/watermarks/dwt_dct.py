...
from ..config import (
    DWTDCT_DEFAULT_BLOCK,
    DWTDCT_DEFAULT_SCALES,
    DWTDCT_MSG_LEN as MESSAGE_LENGTH,
    DWTDCT_THRESHOLD as THRESHOLD,
)
...
class DwtDCT(Watermark):
    def __init__(self, use_svd: bool = False):
        self._scales = list(DWTDCT_DEFAULT_SCALES)
        self._block = DWTDCT_DEFAULT_BLOCK
        self._use_svd = use_svd

    # Watermark ABC -------------------------------------------------------
    def embed(self, image, message):
        return self.encode([image], np.asarray(message)[None, :])[0]
