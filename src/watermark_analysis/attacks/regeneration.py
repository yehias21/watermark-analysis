...
from ..config import SDXL_REFINER_MODEL as DEFAULT_DIFFUSER_MODEL
...
class GenerativeAttack(Attack):
    """Base class for regeneration attacks; operates on batches/loaders."""

    def apply(self, image):  # pragma: no cover - folder-level in practice
        raise NotImplementedError("Use attack(dataloader, output_dir) instead.")


class VAEAttack(GenerativeAttack):
...
class DiffuserAttack(GenerativeAttack):
